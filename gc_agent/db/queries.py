...
def _normalize_referral_invite(row: dict[str, Any]) -> dict[str, Any]:
    """Normalize one referral_invites row into API-safe payload."""
    return {
        "id": str(row.get("id", "")).strip(),
        "gc_id": str(row.get("gc_id", "")).strip(),
        "invite_code": str(row.get("invite_code", "")).strip(),
        "channel": str(row.get("channel", "")).strip() or "link",
        "destination": str(row.get("destination", "")).strip(),
        "invitee_name": str(row.get("invitee_name", "")).strip(),
...
def _normalize_referral_lead(row: dict[str, Any]) -> dict[str, Any]:
    """Normalize one referral_leads row into API-safe payload."""
    return {
        "id": str(row.get("id", "")).strip(),
        "invite_id": str(row.get("invite_id", "")).strip(),
        "gc_id": str(row.get("gc_id", "")).strip(),
        "invite_code": str(row.get("invite_code", "")).strip(),
        "referred_name": str(row.get("referred_name", "")).strip(),
        "referred_contact": str(row.get("referred_contact", "")).strip(),
...
async def insert_referral_invite(
    *,
    gc_id: str,
    channel: str = "link",
    destination: str = "",
    ...
    """Create and persist one referral invite for a contractor account."""
    client = get_client()
    gc_value = gc_id.strip()
    if not gc_value:
        raise DatabaseError("insert_referral_invite failed: gc_id is required")

    invite_id = f"ref-{uuid4().hex[:12]}"
    invite_code = uuid4().hex[:10].upper()
    payload = {
        "id": invite_id,
        "gc_id": gc_value,
        "invite_code": invite_code,
        "channel": channel.strip() or "link",
    ...
    def _query() -> list[dict[str, Any]]:
        response = client.table("referral_invites").insert(payload).execute()
        return list(response.data or [])

    rows = await _run_db("insert_referral_invite", _query)
    if not rows:
        return _normalize_referral_invite(payload)
    return _normalize_referral_invite(rows[0])


async def list_referral_invites(gc_id: str, limit: int = 25) -> list[dict[str, Any]]:
    """List referral invites for a contractor account newest first."""
    client = get_client()
    gc_value = gc_id.strip()
    if not gc_value:
        return []
    ...
    def _query() -> list[dict[str, Any]]:
        response = (
            ...
            .select("id,gc_id,invite_code,channel,destination,invitee_name,note,status,trace_id,created_at,accepted_at")
            .eq("gc_id", gc_value)
            .order("created_at", desc=True)
            .limit(capped_limit)
            .execute()
    ...
    rows = await _run_db("list_referral_invites", _query)
    return [_normalize_referral_invite(row) for row in rows if isinstance(row, dict)]


async def list_referral_leads(gc_id: str, limit: int = 25) -> list[dict[str, Any]]:
    """List accepted referral leads for a contractor account newest first."""
    client = get_client()
    gc_value = gc_id.strip()
    if not gc_value:
        return []
    ...
    def _query() -> list[dict[str, Any]]:
        response = (
            client.table("referral_leads")
            .select("id,invite_id,gc_id,invite_code,referred_name,referred_contact,source,status,metadata,created_at")
            .eq("gc_id", gc_value)
            .order("created_at", desc=True)
            .limit(capped_limit)
            .execute()
    ...
    rows = await _run_db("list_referral_leads", _query)
    return [_normalize_referral_lead(row) for row in rows if isinstance(row, dict)]
...
async def accept_referral_invite(
    *,
    ...
    """Accept a referral code and record the inbound referred lead."""
    client = get_client()
    code_value = invite_code.strip().upper()
    if not code_value:
        raise DatabaseError("accept_referral_invite failed: invite_code is required")

    def _query() -> dict[str, Any]:
        lookup = (
            client.table("referral_invites")
            .select("id,gc_id,invite_code,status")
            .eq("invite_code", code_value)
            .limit(1)
            .execute()
        )
        rows = list(lookup.data or [])
        if not rows:
            raise RuntimeError("invite_code not found")

        invite_row = rows[0]
        invite_id = str(invite_row.get("id", "")).strip()
        gc_value = str(invite_row.get("gc_id", "")).strip()
        if not invite_id or not gc_value:
            raise RuntimeError("invite row missing required ids")

        lead_id = f"rlead-{uuid4().hex[:12]}"
        lead_payload = {
            "id": lead_id,
            "invite_id": invite_id,
            "gc_id": gc_value,
            "invite_code": code_value,
            "referred_name": referred_name.strip() or None,
            "referred_contact": referred_contact.strip() or None,
            "source": source.strip() or None,
            "status": "new",
            "metadata": metadata if isinstance(metadata, dict) else {},
            "created_at": _utcnow_iso(),
        }
        client.table("referral_leads").insert(lead_payload).execute()

        status_value = str(invite_row.get("status", "")).strip().lower()
        ...
        return {
            ...
            "invite_id": invite_id,
            "invite_code": code_value,
            "referrer_gc_id": gc_value,
            "accepted": True,
        }

    return await _run_db("accept_referral_invite", _query)
