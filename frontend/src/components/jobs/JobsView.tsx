...
function JobsViewContent({ jobs, useStore = false }: { jobs: Job[]; useStore?: boolean }) {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs.filter((job) =>
      [job.name, job.customerName, ...(job.tags ?? [])].some((value) => value.toLowerCase().includes(normalized))
    );
  }, [jobs, query]);
  ...
  return (
    <div className="flex h-full overflow-hidden bg-[var(--bg)]">
      <div className="flex w-[300px] shrink-0 flex-col border-r border-[var(--line)] bg-[var(--bg-2)]">
      ...
      <div className="min-w-0 flex-1 overflow-hidden">
        {selectedJob ? (
          <JobDetail job={selectedJob} onSaveNotes={(notes) => updateJobNotes(selectedJob.id, notes)} />
        ) : (
