  ...
  return (
    <section className="grid grid-cols-4 border-b border-[var(--line)]">
      {cards.map((card, index) => (
        <motion.div key={card.label} custom={index} initial="hidden" animate="visible" variants={fadeUp} className={`px-5 py-4 ${index < cards.length - 1 ? "border-r border-[var(--line)]" : ""}`}>
